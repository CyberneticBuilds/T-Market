...
async def process_cart(message: Message, state: FSMContext):
    ...
    if len(cart_data) == 0:
        ...
        await message.answer('Ваша корзинка пуста!')
        ...
        if order_cost != 0:
            ...
            await message.answer('Перейти до оформлення доків?',
                                 reply_markup=markup)
...
async def process_check_cart_invalid(message: Message):
    await message.reply('Такого варіку не було')
...
async def process_check_cart_all_right(message: Message, state: FSMContext):
    ...
    await message.answer('Вкажіть імя',
...
async def process_name(message: Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        if 'address' in data.keys():
            ...
            await CheckoutState.next()
            await message.answer('Вкажіть адрес',
                                 reply_markup=back_markup())
...
async def process_address_back(message: Message, state: FSMContext):
    async with state.proxy() as data:
        await message.answer('Змінити імя з <b>' + data['name'] + '</b>?',
                             reply_markup=back_markup())
...
async def confirm(message):
    await message.answer('Переконайся шо всьо норм, і підтверджуй--->',
...
async def process_confirm_invalid(message: Message):
    await message.reply('Такого варіка нема')
    ...
    if enough_money:
        ...
        async with state.proxy() as data:
            ...
            await message.answer(
                'Ок! Ваше замовлення в дорозі  🚀\nІмя: <b>' + data['name'] + '</b>\nАдрес: <b>' + data[
                    'address'] + '</b>',
                reply_markup=markup)
    else:

        await message.answer('У вас нема бабок на рахунку. Поповни баланс!',
                             reply_markup=markup)
