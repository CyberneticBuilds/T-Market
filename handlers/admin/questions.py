...
async def process_questions(message: Message):
    ...
    if len(questions) == 0:
        ...
        await message.answer('Німа питань')
        ...
        for cid, question in questions:
            ...
            markup.add(InlineKeyboardButton(
                'Дай відповідь', callback_data=question_cb.new(cid=cid, action='answer')))
...
async def process_submit(message: Message, state: FSMContext):
    ...
    await message.answer('Дай гарантію шо не помилився у відповіді', reply_markup=submit_markup())
...
async def process_send_answer(message: Message, state: FSMContext):
    await message.answer('Атмєра Охрана!', reply_markup=ReplyKeyboardRemove())
    await state.finish()
