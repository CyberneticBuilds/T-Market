...
async def process_settings(message: Message):
    ...
    markup.add(InlineKeyboardButton(
        '+ Добавити категорію', callback_data='add_category'))
...
async def category_callback_handler(query: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await query.answer('Всі товари в категориї')
...
async def delete_category_handler(message: Message, state: FSMContext):
    async with state.proxy() as data:
        if 'category_index' in data.keys():
            ...
            await message.answer('Готово чіна!', reply_markup=ReplyKeyboardRemove())
            await process_settings(message)
...
async def process_cancel(message: Message, state: FSMContext):
    await message.answer('Ок, саунтрес ролбек!', reply_markup=ReplyKeyboardRemove())
    await state.finish()
...
async def process_image_photo(message: Message, state: FSMContext):
    ...
    await ProductState.next()
    await message.answer('хав мач?', reply_markup=back_markup())
...
async def process_image_url(message: Message, state: FSMContext):
    if message.text == back_message:
        ...
        await message.answer('Вам треба скинути фото товару!')
...
async def delete_product_callback_handler(query: CallbackQuery, callback_data: dict):
    ...
    await query.answer('Видалено нахуй!')
    await query.message.delete()
...
async def show_products(m, products, category_idx):
    ...
    for idx, title, body, image, price, tag in products:
        ...
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton(
            'Видалити', callback_data=product_cb.new(id=idx, action='delete')))
    ...
    await m.answer('Є бажання шось добавити або видалити?', reply_markup=markup)
